...
function getHandType(hand) {
    const {J: jokers, ...entries} = Array.from(hand).reduce((acc, curr) => (acc[curr] = (acc[curr] ?? 0) + 1, acc), {});
    const values = Object.values(entries).sort((a, b) => b - a);
    values[0] = values[0] ? values[0] + (jokers ?? 0) : jokers;

    return values.reduce((type, occ) => {
        let outp = 1;
