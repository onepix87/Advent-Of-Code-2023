...
for (let i = 0; i < galaxies.length; i++) {
    for (let j = i + 1; j < galaxies.length; j++) {
        distanceSum = distanceSum +
            emptyColumns.filter(val => (val > galaxies[j][0] && val < galaxies[i][0]) || (val < galaxies[j][0] && val > galaxies[i][0])).length * 999999 + Math.abs(galaxies[i][0] - galaxies[j][0]) +
            emptyRows.filter(val => (val > galaxies[j][1] && val < galaxies[i][1]) || (val < galaxies[j][1] && val > galaxies[i][1])).length * 999999 + Math.abs(galaxies[i][1] - galaxies[j][1]);
    }
}
